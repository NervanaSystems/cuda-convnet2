            ...
            if (numFilterColors > 3)  {
                if (numFilterColors % 64 == 0) {
                    if (numFiltersPerGroup % 128 == 0) {
                        if (hidActs.getNumDataBytes() < TEXTURE_SIZE_MAX && images.getNumDataBytes() < TEXTURE_SIZE_MAX) {
                ...
                if (numFilterColors == 3) {
                    if (numFiltersPerGroup % 64 == 0) {
                        if (hidActs.getNumDataBytes() < TEXTURE_SIZE_MAX && images.getNumDataBytes() < TEXTURE_SIZE_MAX) {
                            ...
                            cudaFuncSetCacheConfig(conv_weight_acts_c_kepler_sw < 16, 16, 2, 2, 4, 32, 3, true, false >, cudaFuncCachePreferShared);
                            conv_weight_acts_c_kepler_sw < 16, 16, 2, 2, 4, 32, 3, true, false ><<<blocks, threads, 0, stream>>>(images.getDevData(), hidActs.getDevData(), targets.getDevData(), numImages, numFilters, numModulesY, numModulesX, imgSizeY, imgSizeX, filterSize, paddingStart, moduleStride, imgStride, sumWidth, scaleTargets, scaleOutput);
                        }
                    }
                    else if (numFiltersPerGroup % 48 == 0) {
                        if (hidActs.getNumDataBytes() < TEXTURE_SIZE_MAX && images.getNumDataBytes() < TEXTURE_SIZE_MAX) {
                            ...
                            cudaFuncSetCacheConfig(conv_weight_acts_c_kepler_sw < 16, 16, 2, 4, 3, 32, 3, true, false >, cudaFuncCachePreferShared);
                            conv_weight_acts_c_kepler_sw < 16, 16, 2, 4, 3, 32, 3, true, false ><<<blocks, threads, 0, stream>>>(images.getDevData(), hidActs.getDevData(), targets.getDevData(), numImages, numFilters, numModulesY, numModulesX, imgSizeY, imgSizeX, filterSize, paddingStart, moduleStride, imgStride, sumWidth, scaleTargets, scaleOutput);
                        }
                    }
                    else if (numFiltersPerGroup % 32 == 0) {
