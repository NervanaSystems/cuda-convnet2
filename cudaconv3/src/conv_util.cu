 ...
 *
 * * Fixed a whitening bug related to NaN/Inf
 * * Added L2 pooling
 * ---------------------------------------------------------------------------
 */
 ...
 * threadIdx.y determines filter idx
 * 
 * poolin:        (numFilters, imgPixels, numImages)
 * poolout:      (numFilters, numOutputs, numImages)
 * doUnpool means a forward operation, so scaleTargets and scaleOutputs are disregarded
 * numImages must be divisible by B_X*imgsPerThread if checkCaseBounds is false
 */
template<int B_Y, int B_X, int imgsPerThread, int filtersPerThread, bool checkCaseBounds, bool add>
__global__ void kLocalUnpool(float* largeTgt, float* smallTgt, const int numFilters, const int numImages, const int subsX,
                            const int largeX, const int smallX, bool doUnpool, const float scaleTargets, const float scaleOutputs) {
    ...
    const int outputIdx = outputIdxY * smallX + outputIdxX;
    const int numOutputs = smallX * smallX;
    const int imgPixels = largeX * largeX;
    
    const int startImgPxX = outputIdxX * subsX;
    const int startImgPxY = outputIdxY * subsX;
    const int imgIdx = blockImgIdx + threadIdx.x;
    
    largeTgt += myFilterIdx * imgPixels * numImages + imgIdx;
    smallTgt += (myFilterIdx * numOutputs + outputIdx) * numImages + imgIdx;

    const int loopStartY = MAX(0, startImgPxY);
    const int loopStartX = MAX(0, startImgPxX);
    const int loopEndY = MIN(largeX, startImgPxY + subsX);
    const int loopEndX = MIN(largeX, startImgPxX + subsX);

    if (!doUnpool) {
        const int imgPx = loopStartY * largeX + loopStartX;
        if (!add) {
            #pragma unroll
            for (int i = 0; i < imgsPerThread; i++) {
                if (!checkCaseBounds || imgIdx + i * B_X < numImages) {
                    #pragma unroll
                    for (int f = 0; f < filtersPerThread; f++) {
                        smallTgt[f * numOutputs * numImages + i * B_X] = largeTgt[(f * imgPixels + imgPx) * numImages + i * B_X];
                    }
                }
            }
        } else {
            for (int i = 0; i < imgsPerThread; i++) {
                if (!checkCaseBounds || imgIdx + i * B_X < numImages) {
                    #pragma unroll
                    for (int f = 0; f < filtersPerThread; f++) {
                        smallTgt[f * numOutputs * numImages + i * B_X] = scaleTargets * smallTgt[f * numOutputs * numImages + i * B_X] + 
                            scaleOutputs * largeTgt[(f * imgPixels + imgPx) * numImages + i * B_X];
                    }
                }
            }
        }
        ...
        for (int y = loopStartY; y < loopEndY; y++) {
            for (int x = loopStartX; x < loopEndX; x++) {
                const int imgPx = y * largeX + x;
                #pragma unroll
                for (int i = 0; i < imgsPerThread; i++) {
                    if (!checkCaseBounds || imgIdx + i * B_X < numImages) {
                        #pragma unroll
                        for (int f = 0; f < filtersPerThread; f++) {
                            if (y == loopStartY && x == loopStartX)
                                largeTgt[(f * imgPixels + imgPx) * numImages + i * B_X] = smallTgt[f * numOutputs * numImages + i * B_X];
                            else
                                largeTgt[(f * imgPixels + imgPx) * numImages + i * B_X] = 0.0f;
                        }
                    }
                }
            }
 ...
 */
template<int B_Y, int B_X, int imgsPerThread, int filtersPerThread, bool add, bool checkCaseBounds>
__global__ void kLocalL2Undo(float* imgs, float* l2Grads, float* l2Acts, float* target, const int imgSize, const int numFilters,
                              const int numImages, const int subsX, const int startX, const int strideX, const int outputsX,
                              const float scaleTargets, const float scaleOutputs) {
    __shared__ float shImgs[B_Y*filtersPerThread][B_X*imgsPerThread];
    const int numImgBlocks = DIVUP(numImages,B_X*imgsPerThread);
    ...
    imgs += ((blockFilterIdx + threadIdx.y) * imgPixels + blockPx) * numImages + imgIdx;
    l2Grads += ((blockFilterIdx + threadIdx.y) * numOutputs) * numImages
            + imgIdx;
    l2Acts += ((blockFilterIdx + threadIdx.y) * numOutputs) * numImages
            + imgIdx;
            ...
            for (int mx = startOutputX; mx < endOutputX; mx++) {
                ...
                for (int i = 0; i < imgsPerThread; i++) {
                    if (!checkCaseBounds || imgIdx + i * B_X < numImages) {
                        #pragma unroll
                        for (int f = 0; f < filtersPerThread; f++) {
                            const float ma = l2Acts[(f * B_Y * numOutputs + outputIdx) * numImages + i * B_X];
                            const float mg = l2Grads[(f * B_Y * numOutputs + outputIdx) * numImages + i * B_X];
                            const float img = shImgs[threadIdx.y + B_Y * f][threadIdx.x + B_X * i];

                            prod[f][i] += ma == 0 ? 0 : (img/ma) * mg * regionSizeInv * regionSizeInv;
                        }
                    }
                }
            }
...
//    assert(numImages % 128 == 0);
    ...
    if (imgsPerThread == 4) {
        if (checkCaseBounds) {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalAvgUndo<4, 32, 4, 4, false, true><<<blocks, threads, 0, stream>>>(avgGrads.getDevData(), target.getDevData(),
                                                                       imgSize, numFilters, numImages, subsX, startX, strideX,
                                                                       outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalAvgUndo<4, 32, 4, 4, true, true><<<blocks, threads, 0, stream>>>(avgGrads.getDevData(), target.getDevData(),
                                                                      imgSize, numFilters, numImages, subsX, startX, strideX,
                                                                      outputsX, scaleTargets, scaleOutput);
            }
        } else {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalAvgUndo<4, 32, 4, 4, false, false><<<blocks, threads, 0, stream>>>(avgGrads.getDevData(), target.getDevData(),
                                                                       imgSize, numFilters, numImages, subsX, startX, strideX,
                                                                       outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalAvgUndo<4, 32, 4, 4, true, false><<<blocks, threads, 0, stream>>>(avgGrads.getDevData(), target.getDevData(),
                                                                      imgSize, numFilters, numImages, subsX, startX, strideX,
                                                                      outputsX, scaleTargets, scaleOutput);
            }
        }
        ...
        if (checkCaseBounds) {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalAvgUndo<4, 32, 2, 4, false, true><<<blocks, threads, 0, stream>>>(avgGrads.getDevData(), target.getDevData(),
                                                                       imgSize, numFilters, numImages, subsX, startX, strideX,
                                                                       outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalAvgUndo<4, 32, 2, 4, true, true><<<blocks, threads, 0, stream>>>(avgGrads.getDevData(), target.getDevData(),
                                                                      imgSize, numFilters, numImages, subsX, startX, strideX,
                                                                      outputsX, scaleTargets, scaleOutput);
            }
        } else {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalAvgUndo<4, 32, 2, 4, false, false><<<blocks, threads, 0, stream>>>(avgGrads.getDevData(), target.getDevData(),
                                                                       imgSize, numFilters, numImages, subsX, startX, strideX,
                                                                       outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalAvgUndo<4, 32, 2, 4, true, false><<<blocks, threads, 0, stream>>>(avgGrads.getDevData(), target.getDevData(),
                                                                      imgSize, numFilters, numImages, subsX, startX, strideX,
                                                                      outputsX, scaleTargets, scaleOutput);
            }
        }
        ...
        if (checkCaseBounds) {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalAvgUndo<4, 32, 1, 4, false, true><<<blocks, threads, 0, stream>>>(avgGrads.getDevData(), target.getDevData(),
                                                                       imgSize, numFilters, numImages, subsX, startX, strideX,
                                                                       outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalAvgUndo<4, 32, 1, 4, true, true><<<blocks, threads, 0, stream>>>(avgGrads.getDevData(), target.getDevData(),
                                                                      imgSize, numFilters, numImages, subsX, startX, strideX,
                                                                      outputsX, scaleTargets, scaleOutput);
            }
        } else {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalAvgUndo<4, 32, 1, 4, false, false><<<blocks, threads, 0, stream>>>(avgGrads.getDevData(), target.getDevData(),
                                                                       imgSize, numFilters, numImages, subsX, startX, strideX,
                                                                       outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalAvgUndo<4, 32, 1, 4, true, false><<<blocks, threads, 0, stream>>>(avgGrads.getDevData(), target.getDevData(),
                                                                      imgSize, numFilters, numImages, subsX, startX, strideX,
                                                                      outputsX, scaleTargets, scaleOutput);
            }
        }
...
void convLocalL2Undo(NVMatrix& images, NVMatrix& l2Grads, NVMatrix& l2Acts, NVMatrix& target,
                      int subsX, int startX, int strideX, int outputsX) {
    convLocalL2Undo(images, l2Grads, l2Acts, target, subsX, startX, strideX, outputsX, 0, 1);
}

/*
 * imgs:        (numFilters, imgPixels, numImages)
 * l2Grads:    (numFilters, numOutputs, numImages)
 * rL2Acts:    (numFilters, numOutputs, numImages)
 * target:      (numFilters, imgPixels, numImages)
 */
void convLocalL2Undo(NVMatrix& images, NVMatrix& l2Grads, NVMatrix& l2Acts, NVMatrix& target,
                      int subsX, int startX, int strideX, int outputsX, float scaleTargets, float scaleOutput) {
    int outputs = outputsX * outputsX;
    int numImages = images.getNumCols();
    int numFilters = l2Grads.getNumRows() / outputs;
    int imgPixels = images.getNumRows() / numFilters;
    assert(images.getNumRows() == numFilters * imgPixels);
    int imgSize = int(sqrt(imgPixels));
    
    assert(imgSize * imgSize == imgPixels);
    assert(l2Grads.getNumRows() == numFilters * outputs);
    assert(l2Grads.getNumCols() == numImages);
    assert(!images.isTrans());
    assert(!target.isTrans());
    assert(!l2Grads.isTrans());
    assert(!l2Acts.isTrans());
    assert(images.isContiguous());
    assert(l2Grads.isContiguous());
    assert(l2Acts.isContiguous());
    assert(l2Grads.isSameDims(l2Acts));
    assert(numFilters % 16 == 0);
    ...
    if (imgsPerThread == 4) {
        if  (checkCaseBounds) {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalL2Undo<4, 32, 4, 2, false, true><<<blocks, threads, 0, stream>>>(images.getDevData(), l2Grads.getDevData(), l2Acts.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalL2Undo<4, 32, 4, 2, true, true><<<blocks, threads, 0, stream>>>(images.getDevData(), l2Grads.getDevData(), l2Acts.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            }
        } else {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalL2Undo<4, 32, 4, 2, false, false><<<blocks, threads, 0, stream>>>(images.getDevData(), l2Grads.getDevData(), l2Acts.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalL2Undo<4, 32, 4, 2, true, false><<<blocks, threads, 0, stream>>>(images.getDevData(), l2Grads.getDevData(), l2Acts.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            }
        }
    } else if (imgsPerThread == 2) {
        if  (checkCaseBounds) {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalL2Undo<4, 32, 2, 2, false, true><<<blocks, threads, 0, stream>>>(images.getDevData(), l2Grads.getDevData(), l2Acts.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalL2Undo<4, 32, 2, 2, true, true><<<blocks, threads, 0, stream>>>(images.getDevData(), l2Grads.getDevData(), l2Acts.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            }
        } else {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalL2Undo<4, 32, 2, 2, false, false><<<blocks, threads, 0, stream>>>(images.getDevData(), l2Grads.getDevData(), l2Acts.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalL2Undo<4, 32, 2, 2, true, false><<<blocks, threads, 0, stream>>>(images.getDevData(), l2Grads.getDevData(), l2Acts.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            }
        }
    } else {
        if  (checkCaseBounds) {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalL2Undo<4, 32, 1, 2, false, true><<<blocks, threads, 0, stream>>>(images.getDevData(), l2Grads.getDevData(), l2Acts.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalL2Undo<4, 32, 1, 2, true, true><<<blocks, threads, 0, stream>>>(images.getDevData(), l2Grads.getDevData(), l2Acts.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            }
        } else {
            if (scaleTargets == 0 && scaleOutput == 1) {
                kLocalL2Undo<4, 32, 1, 2, false, false><<<blocks, threads, 0, stream>>>(images.getDevData(), l2Grads.getDevData(), l2Acts.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            } else {
                kLocalL2Undo<4, 32, 1, 2, true, false><<<blocks, threads, 0, stream>>>(images.getDevData(), l2Grads.getDevData(), l2Acts.getDevData(), target.getDevData(),
                                                                imgSize, numFilters, numImages, subsX, startX, strideX, outputsX, scaleTargets, scaleOutput);
            }
        }
    }

    getLastCudaError("convLocalL2Undo: kernel execution failed");
}
...
void convLocalUnpoolForward(NVMatrix& smallImages, NVMatrix& bigImages, int numFilters, int subsX, int smallX, int largeX) {
    convLocalUnpool(bigImages, smallImages, numFilters, subsX, smallX, largeX, true, 0, 1);
}

void convLocalUnpoolBackward(NVMatrix& bigGrads, NVMatrix& smallGrads, int numFilters, int subsX, int smallX, int largeX) {
    convLocalUnpoolBackward(bigGrads, smallGrads, numFilters, subsX, smallX, largeX, 0, 1);
}

void convLocalUnpoolBackward(NVMatrix& bigGrads, NVMatrix& smallGrads, int numFilters, int subsX, int smallX, int largeX, float scaleTargets, float scaleOutput) {
    convLocalUnpool(bigGrads, smallGrads, numFilters, subsX, smallX, largeX, false, scaleTargets, scaleOutput);
}

void convLocalUnpool(NVMatrix& largeTgt, NVMatrix& smallTgt, int numFilters, int subsX, int smallX, int largeX, bool doUnpool, float scaleTargets, float scaleOutput) {

    /* 
     * In the context of this operation, largeTgt is the large matrix, and smallTgt is the small matrix
     * we are unpooling small to big
     * we say that largeTgt have 
     */

    int outputs = smallX * smallX;
    int imgPixels = largeX * largeX;
    int numImages = doUnpool ? smallTgt.getNumCols() : largeTgt.getNumCols();

    if (doUnpool)
        largeTgt.resize(numFilters * imgPixels, numImages);
    else 
        smallTgt.resize(numFilters * outputs, numImages);

    assert(!largeTgt.isTrans());
    assert(!smallTgt.isTrans());
    assert(largeTgt.isContiguous());
    assert(smallTgt.isContiguous());
    cudaStream_t stream = NVMatrix::getDefaultStream();
    ...
    bool checkCaseBounds = numImages % (32*imgsPerThread) != 0;
    dim3 threads(32, 4);
    dim3 blocks(DIVUP(numImages,32*imgsPerThread) * smallX, DIVUP(numFilters, 4 * filtersPerThread) * smallX);
    if (imgsPerThread == 4) {
        if (filtersPerThread == 1) {
            if (checkCaseBounds) {
                if (scaleTargets == 0 && scaleOutput == 1) {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 4, 1, true, false>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 4, 1, true, false><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                } else {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 4, 1, true, true>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 4, 1, true, true><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                }
            } else {
                if (scaleTargets == 0 && scaleOutput == 1) {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 4, 1, false, false>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 4, 1, false, false><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                } else {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 4, 1, false, true>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 4, 1, false, true><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                }
            }
            ...
            if (checkCaseBounds) {
                if (scaleTargets == 0 && scaleOutput == 1) {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 4, 4, true, false>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 4, 4, true, false><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                } else {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 4, 4, true, true>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 4, 4, true, true><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                }
            } else {
                if (scaleTargets == 0 && scaleOutput == 1) {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 4, 4, false, false>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 4, 4, false, false><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                } else {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 4, 4, false, true>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 4, 4, false, true><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                }
            }
        ...
        if (filtersPerThread == 1) {
            if (checkCaseBounds) {
                if (scaleTargets == 0 && scaleOutput == 1) {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 2, 1, true, false>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 2, 1, true, false><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                } else {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 2, 1, true, true>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 2, 1, true, true><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                }
            } else {
                if (scaleTargets == 0 && scaleOutput == 1) {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 2, 1, false, false>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 2, 1, false, false><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                } else {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 2, 1, false, true>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 2, 1, false, true><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                }
            }
            ...
            if (checkCaseBounds) {
                if (scaleTargets == 0 && scaleOutput == 1) {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 2, 4, true, false>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 2, 4, true, false><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                } else {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 2, 4, true, true>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 2, 4, true, true><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                }
            } else {
                if (scaleTargets == 0 && scaleOutput == 1) {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 2, 4, false, false>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 2, 4, false, false><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                } else {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 2, 4, false, true>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 2, 4, false, true><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                }
            }
        ...
        if (filtersPerThread == 1) {
            if (checkCaseBounds) {
                if (scaleTargets == 0 && scaleOutput == 1) {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 1, 1, true, false>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 1, 1, true, false><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                } else {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 1, 1, true, true>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 1, 1, true, true><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                }
            } else {
                if (scaleTargets == 0 && scaleOutput == 1) {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 1, 1, false, false>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 1, 1, false, false><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                } else {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 1, 1, false, true>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 1, 1, false, true><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                }
            }
            ...
            if (checkCaseBounds) {
                if (scaleTargets == 0 && scaleOutput == 1) {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 1, 4, true, false>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 1, 4, true, false><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                } else {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 1, 4, true, true>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 1, 4, true, true><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                }
            } else {
                if (scaleTargets == 0 && scaleOutput == 1) {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 1, 4, false, false>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 1, 4, false, false><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                } else {
                    cudaFuncSetCacheConfig(kLocalUnpool<4, 32, 1, 4, false, true>, cudaFuncCachePreferL1);
                    kLocalUnpool<4, 32, 1, 4, false, true><<<blocks, threads, 0, stream>>>(largeTgt.getDevData(), smallTgt.getDevData(),
                                                                      numFilters, numImages, subsX, largeX, smallX,
                                                                      doUnpool, scaleTargets, scaleOutput);
                }
            }
    ...
    getLastCudaError("convLocalUnpool: kernel execution failed");
}
/*
 * images:      (numFilters, imgPixels, numImages)
 * meanDiffs:   (numFilters, imgPixels, numImages)
