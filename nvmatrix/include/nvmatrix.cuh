 ...
 *
 * * Quanitization support, filter normalization
 * * Added agmin, argmax kernels, fleshed out other operations for cudanet
 *   support
 * * squared cost layer
 * ---------------------------------------------------------------------------
 */
...
public:
    ...
    static int getNumCublasHandles() {return _cublasHandles.count(getDeviceID());}
    ...
    void addProductRM(NVMatrix& a, NVMatrix &b, float scaleThis, float scaleAB, bool transA, bool transB, cudaStream_t stream);
    void addProductRM(NVMatrix& a, NVMatrix &b, float scaleThis, float scaleAB, bool transA, bool transB);
    ...
    void addUniformNoise(float minRange, float maxRange);
    void addUniformNoiseBinarize(float prob, float precision);
    ...
    void argmin(int axis, NVMatrix& target, cudaStream_t stream);
    ...
    float mean();
    float sumsq();
    float sumsq(NVMatrix& tmpbuf);
    float sum();
    float sum(NVMatrix& tmpbuf);
    float meanabs();
    float sumabs();
    float sumabs(NVMatrix& tmpbuf);
    float max();
    float maxabs();
    float min();
    float countInf();
    ...
    void quantizeValues(int iw, int fw, NVMatrix& target);
    void quantizeValues(int iw, int fw);
    void quantizeValues(int* wts);
    ...
    void assign(float _value);
    void assignSlice(int startRow, int endRow, int startCol, int endCol, float _value);
    void zero();
