 ...
 *
 * * Added agmin, argmax kernels
 * * Additive uniform random number generation
 * * Build stability on OSX
 * ---------------------------------------------------------------------------
 */
...
#if defined(_WIN64) || defined(_WIN32) || defined(_OSX)
...
class AddUniformUnaryRandomizer {
private:
    const float minRange;
    const float maxRange;
public:
    AddUniformUnaryRandomizer(float _minRange, float _maxRange) : minRange(_minRange), maxRange(_maxRange) {
    }
    __device__ inline float operator ()(float data, curandState* state) {
        return data + curand_uniform(state)*(maxRange - minRange)+minRange;
    }
};

class AddUniformUnaryBinarizer {
private:
    const float prob;
    const float precision;
public:
    AddUniformUnaryBinarizer(float _prob, float _precision) : prob(_prob), precision(_precision) {
    }
    __device__ inline float operator ()(float data, curandState* state) {
        float rndnum = curand_uniform(state);

        return data + (rndnum > prob ? (rndnum < 1-prob ? 0 : precision) : -precision);
    }
};
