 ...
 *
 * * Quanitization support, filter normalization, bit sliding
...
void NVMatrix::addProductRM(NVMatrix& a, NVMatrix &b, float scaleThis, float scaleAB, bool transA, bool transB) {
    addProductRM(a, b, scaleThis, scaleAB, transA, transB, getDefaultStream());
}
/*
 * This will only work if this matrix is in row-major order! In other words,
 * if isTrans() returns false.
 * transA and transB are from the point of view of the calling prog
 */
void NVMatrix::addProductRM(NVMatrix& a, NVMatrix &b, float scaleThis, float scaleAB, bool transA, bool transB, cudaStream_t stream) {
    int ka = transA ? a.getNumRows() : a.getNumCols();
    int m = transA ? a.getNumCols() : a.getNumRows();
    int n = transB ? b.getNumRows() : b.getNumCols();
    int kb = transB ? b.getNumCols() : b.getNumRows();
    // printf("A:%dx%d, m: %d, ka: %d    B:%dx%d, n: %d, kb: %d", a.getNumRows(), a.getNumCols(), m, ka, b.getNumRows(), b.getNumCols(), n, kb);
    assert(ka == kb);
    int lda = a.getStride();
    int ldb = b.getStride();
    if (scaleThis == 0) {
    ...
    assert(this->getNumRows() == m);
    assert(this->getNumCols() == n);
    CUBLAS_CALL(cublasSetStream_v2(getCublasHandle(), stream));
    CUBLAS_CALL(cublasSgemm_v2(getCublasHandle(), !transB ? CUBLAS_OP_N : CUBLAS_OP_T, !transA ? CUBLAS_OP_N : CUBLAS_OP_T, n, m, ka,
                               &scaleAB, b.getDevData(), ldb, a.getDevData(), lda,
                               &scaleThis, getDevData(), n));
}
...
void NVMatrix::addUniformNoise(float minRange, float maxRange) {
    _unaryRandomize(*this, AddUniformUnaryRandomizer(minRange, maxRange));
}

void NVMatrix::addUniformNoiseBinarize(float prob, float precision){
    _unaryRandomize(*this, AddUniformUnaryBinarizer(prob, precision));
}
...
bool NVMatrix::resize(int numRows, int numCols, bool trans) {
    ...
    if (numRows != _numRows || numCols != _numCols) {
        // not sure which assert fails, so break this down:
        //printf("debug nvmatrix resize: (Owns check: %d) OR (Element check: %d AND Contiguos check: %d) \n", _ownsData, _numElements == numRows * numCols, isContiguous());
        // original code
...
void NVMatrix::quantizeValues(int iw, int fw, NVMatrix& target) {
    apply(NVMatrixOps::Quantize(iw, fw), target);
}

void NVMatrix::quantizeValues(int iw, int fw) {
    quantizeValues(iw, fw, *this);
}

void NVMatrix::quantizeValues(int *wts) {
//     if (0) {
//         float tmin = this->min();
//         int tmax = (int) ceil(log2(std::max(this->max(), fabs(tmin))));
//         if (tmax!=wts[0]) {
//             wts[0] = tmax;
// //        wts[1] = abs(wts[0]-15);
//             if (wts[0]>wts[3]) {
...
//     }
    if (wts[2]==3) {
        float tmaxabs = this->maxabs();
        float tmeanabs = this->meanabs();
        printf("AvgMag: %f,%d MaxMag: %f,%d \n", tmeanabs, (int) ceil(log2(tmeanabs)), tmaxabs, (int) ceil(log2(tmaxabs)));
        return;
    } else {
        quantizeValues(wts[0], abs(wts[0]-wts[1]), *this);
    }
    if (wts[2] == 4) {
        print(1,5);
    }
...
void NVMatrix::assign(float _value) {
    apply(NVMatrixOps::Value(_value));
}

void NVMatrix::assignSlice(int startRow, int endRow, int startCol, int endCol, float _value) {
    slice(startRow, endRow, startCol, endCol).assign(_value);
}
...
void NVMatrix::argmax(int axis, NVMatrix& target) {
    argmax(axis, target, NVMatrix::getDefaultStream());
}
void NVMatrix::argmin(int axis, NVMatrix& target) {
    argmin(axis, target, NVMatrix::getDefaultStream());
}

void NVMatrix::argmax(int axis, NVMatrix& target, cudaStream_t stream) {
    int h = getNumRows();
    int w = getNumCols();
    if (axis==0) {
        kArgMaxColumnwise<<<w,32, 0, stream>>>(getDevData(), target.getDevData(), w, h);
    } else {
        kArgMaxRowwise<<<h,32, 0, stream>>>(getDevData(), target.getDevData(), w, h);
    }
}

void NVMatrix::argmin(int axis, NVMatrix& target, cudaStream_t stream) {
    int h = getNumRows();
    int w = getNumCols();
    if (axis==0) {
        kArgMinColumnwise<<<w,32, 0, stream>>>(getDevData(), target.getDevData(), w, h);
    } else {
        kArgMinRowwise<<<h,32, 0, stream>>>(getDevData(), target.getDevData(), w, h);
    }
}
...
void NVMatrix::mean(int axis, NVMatrix& target) {
    float divisor = axis == 0 ? (float) getNumRows() : (float) getNumCols();
    sum(axis, target, getDefaultStream());
    target.scale(1./divisor);
}

void NVMatrix::var(int axis, NVMatrix &mean, NVMatrix& target) {
    // Assumes you have already calculated mean
    float divisor = axis == 0 ? (float) getNumRows() : (float) getNumCols();
    sumOfSquares(axis, target, getDefaultStream());
    target.scale(1./divisor);
    target.applyBinary(NVMatrixBinaryOps::SubSquared(), mean, target);
}
...
float NVMatrix::sumsq() {
    return _totalAgg(NVMatrixAggs::SumSq());
}

float NVMatrix::sumsq(NVMatrix& tmpbuf) {
    return _totalAgg(NVMatrixAggs::SumSq(), tmpbuf, getDefaultStream());
}
...
float NVMatrix::meanabs() {
    return sumabs() / getNumElements();
}

float NVMatrix::sumabs() {
    return _totalAgg(NVMatrixAggs::SumAbs());
}

float NVMatrix::sumabs(NVMatrix& tmpbuf) {
    return _totalAgg(NVMatrixAggs::SumAbs(), tmpbuf, getDefaultStream());
}
...
float NVMatrix::maxabs() {
    return _totalAgg(NVMatrixAggs::MaxAbs());
}
...
float NVMatrix::cpuAgg(Agg agg, cudaStream_t stream) {
    ...
    if (getNumElements() > 1) { // Sum remainder on CPU
        if (typeid(Agg) == typeid(NVMatrixAggs::Sum)) {
            return bufCPU.sum();
        } else if (typeid(Agg) == typeid(NVMatrixAggs::SumAbs)) {
            return bufCPU.sumabs();
        } else if (typeid(Agg) == typeid(NVMatrixAggs::SumSq)) {
            return bufCPU.sum();
        } else if (typeid(Agg) == typeid(NVMatrixAggs::Max)) {
            return bufCPU.max();
        } else if (typeid(Agg) == typeid(NVMatrixAggs::MaxAbs)) {
            return bufCPU.maxabs();
