 ...
 *
 * * Added sumabs, maxabs, save functionality
...
void Matrix::_initpy(PyArrayObject *src) {
    this->_data = NULL;
    this->_trans = CblasNoTrans;
    if (src != NULL) {
        this->_updateDims(PyArray_DIM(src,0), PyArray_DIM(src,1));
        if (PyArray_ISONESEGMENT(src) && PyArray_NDIM(src)!=0) { 
            this->_data = (MTYPE*) PyArray_DATA(src);
            this->_ownsData = false;
            this->_trans = PyArray_CHKFLAGS(src, NPY_ARRAY_C_CONTIGUOUS) ? CblasNoTrans : CblasTrans;
        } else {
...
Matrix::Matrix(PyArrayObject *src) {
    _initpy(src);
}
...
MTYPE Matrix::maxabs() const {
    return _aggregate(&_maxabs, 0);
}
...
MTYPE Matrix::sumabs() const {
    return _aggregate(&_addabs, 0);
}
