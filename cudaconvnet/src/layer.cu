 ...
 *
 * * Added quantization for activations and gradients
 * * Added constant matrix backprop and probabilistic rounding for some layers
 * * Added flags to track and print bit sliding
 * ---------------------------------------------------------------------------
 */
...
Layer::Layer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID, bool trans) :
    ...
    intv& awidth = *pyDictGetIntV(paramsDict, "actWidth");
    intv& gwidth = *pyDictGetIntV(paramsDict, "gradWidth");
    for (int i=0; i<5; i++) {
        _actWidth[i] = awidth[i];
        _gradWidth[i] = gwidth[i];
    }
    _numReplicasPrev = 1;
    _rcvdBInputMsgs = 0;
...
void Layer::quantizeActs() {
    if (_actWidth[0]!=32 || _actWidth[1]!=32) {
        getActs().quantizeValues(_actWidth);
    }
}

void Layer::quantizeGrads() {
    if (_gradWidth[0]!=32 || _gradWidth[1]!=32) {
        getActsGrad().quantizeValues(_gradWidth);
    }
}
...
//        printf("Layer %s[%d] bprop\n", _name.c_str(), getReplicaID());
        ...
        // QUANT: quantize activations grads here?  
        quantizeGrads();
...
WeightLayer::WeightLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID, bool trans, bool useGrad) :
    ...
    intv& dwidth = *pyDictGetIntV(paramsDict, "deltaWidth");
    intv& wwidth = *pyDictGetIntV(paramsDict, "weightWidth");
    intv& bwidth = *pyDictGetIntV(paramsDict, "biasWidth");
    for (int i = 0; i < 5; i++) {
        _deltaWidth[i] = dwidth[i];
        _weightWidth[i] = wwidth[i];
        _biasWidth[i] = bwidth[i];
    }

    _useConstMatrixStd = pyDictGetFloat(paramsDict, "constMatrixStd");
...
void WeightLayer::_quantizeWeights() {
    for (int i = 0; i < _weights->getSize(); i++) {
        if (_weightWidth[0]!=32 || _weightWidth[1]!=32) {
            _weights->at(i).getW().quantizeValues(_weightWidth);

            if (_weightWidth[4]!=0) {
                assert(_weightWidth[4]>0);
                float lastBitPrec = powf(2.0,-abs(_weightWidth[0]-_weightWidth[1]));
                float flipProb = 1/(float) _weightWidth[4];
                _weights->at(i).getW().addUniformNoiseBinarize(flipProb, lastBitPrec);
            }   
        } 
    }
    if (_biasWidth[0]!=32 || _biasWidth[1]!=32) {
        _biases->getW().quantizeValues(_biasWidth);
    }
}
...
void ConvLayer::bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType) {
    NVMatrix& wtsToUse = _weights->at(inpIdx).getConstW();
    convImgActs(v, wtsToUse, _prev[replicaIdx][inpIdx]->getActsGrad(), _imgSize->at(inpIdx), _imgSize->at(inpIdx), _modulesX,
...
void ConvLayer::_constrainWeights() {
    for (int i = 0; i < _weights->getSize(); i++) {
        ...
        if (_weights->at(i).getWBall() > 0 && _weights->at(i).isOwner() && _weights->at(i).getLearningRateSchedule().getBaseValue() > 0) {
            _weights->at(i).getW().sumOfSquares(0, _norm2);
            _norm2.apply(MaxWeightConstraintOperator(_weights->at(i).getWBall()));
            _weights->at(i).getW().eltwiseMultByVector(_norm2);
        }
    }
}
...
PoolLayer& PoolLayer::make(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID) {
    ...
    if (_pool == "max") {
        ...
        return *new L2PoolLayer(convNetThread, paramsDict, replicaID);        
    }
 ...
 * =====================
 */
L2PoolLayer::L2PoolLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID) : PoolLayer(convNetThread, paramsDict, replicaID, false) {
}

void L2PoolLayer::fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx) {
    convLocalPool(*_inputs[0], getActs(), _channels, _sizeX, _start, _stride, _outputsX, L2Pooler());
}

void L2PoolLayer::bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType) {
    convLocalL2Undo(*_inputs[0], v, getActs(), _prev[replicaIdx][inpIdx]->getActsGrad(), _sizeX, _start, _stride, _outputsX, scaleTargets, 1);
}
...
: Layer(convNetThread, paramsDict, replicaID, false), TwoDLayerInterface(paramsDict) {
    ...
    printf("Response Norm %s Act Widths:", _name.c_str());
    for (int i=0; i<5; i++) {
        printf("%d ", _actWidth[i]);
    }
    printf("\n");
}
...
void SumOfSquaresCostLayer::fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx) {
    //printf("SUMSQ Layer input (%dx%d) acts (%dx%d)\n", _inputs[0]->getNumRows(), _inputs[0]->getNumCols(), getActs().getNumRows(), getActs().getNumCols());
