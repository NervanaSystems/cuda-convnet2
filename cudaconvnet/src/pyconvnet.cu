 ...
 * * fixes for numpy deprecation warnings
 * * Added easier matrix testing
 * ---------------------------------------------------------------------------
 */
...
static PyMethodDef _ConvNetMethods[] = {{ "initModel",          initModel,              METH_VARARGS },
                                        { "easyTest",           easyTest,               METH_VARARGS },
...
PyObject* easyTest(PyObject *self, PyObject *args) {
    ...
    NVMatrix::initRandom();
    NVMatrix tmp(matSize,matSize);
    tmp.randomizeGaussian();
    tmp.print(matSize,matSize);
    int shape[2];
    shape[0] = extraInt;
    shape[1] = 15;
    NVMatrix tmp2(tmp, true);
    tmp.quantizeValues(shape);
    cout << endl;
    tmp.print(matSize, matSize);
    float prec = powf(2.0,-lastBit);
    // tmp.addGaussianNoise(prec);
    tmp2.addUniformNoiseBinarize(0.25, prec);
    cout << "Noised" << endl;
    tmp2.print(matSize, matSize);
    cout << "Quantized" << endl;
    tmp2.quantizeValues(shape);
    tmp2.print(matSize, matSize);    
    cout << endl;
    tmp.addUniformNoiseBinarize(0.25, prec);
    tmp.print(matSize, matSize);
    return Py_BuildValue("iii", matSize, extraInt, 0);
}
