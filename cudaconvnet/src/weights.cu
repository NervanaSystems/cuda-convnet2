...
NVMatrix& Weights::getConstW() const {
    assert(_onGPU);
    if (_constMatrix != NULL)
        return *_constMatrix;
    else
...
void Weights::copyToGPU() {
    ...
    if (_srcWeights == NULL) {
        ...
        if (_parent->getConstMatrixStd()!=0) {
            _constMatrix = _constMatrix == NULL ? new NVMatrix(*_weights) : _constMatrix;
            _constMatrix->scale(_parent->getConstMatrixStd());
            printf ("Finished making constMatrix, CONST Standard Deviation is %f\n", _parent->getConstMatrixStd());
        }
        if (_useGrad) {
...
void Weights::aggregateReplicaGradients(float progress) {
    ...
    NVMatrix::setDeviceID(getDeviceID());
    // Insert the rounding factor here.
