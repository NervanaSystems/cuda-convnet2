 ...
 *
 * * Added quantization and filter normalization related items
 * * Added flags for tracking and printing bit sliding
...
protected:
    ...
    virtual void quantizeActs(); 
    virtual void quantizeGrads(); 
    ...
    inline void maybePrintWidths(std::string& layerName, int* widths, const char *abw) {
        if (widths[4]) {
            printf("\n\t*** QSUM %s %s: %d.%d  (min/max): %2d %2d\n", layerName.c_str(), abw, widths[0], abs(widths[0]-widths[1]), widths[2], widths[3]);
            widths[4] = 0;
        }
    }
public:
    ...
    virtual inline float getConstMatrixStd() {return 0.0;}
    virtual inline int doAdaDelta() {return 0;}
    int getReplicaID();
    int getNumReplicas();
...
public:
    ...
    virtual bool postInit();
    virtual inline float getConstMatrixStd() {return _useConstMatrixStd;}
    virtual inline int doAdaDelta() {return _doAdaDelta;}
};
...
class L2PoolLayer : public PoolLayer {
protected:
    void fpropActs(int inpIdx, float scaleTargets, PASS_TYPE passType, int passIdx);
    void bpropActs(NVMatrix& v, int replicaIdx, int inpIdx, float scaleTargets, PASS_TYPE passType);
public:
    L2PoolLayer(ConvNetThread* convNetThread, PyObject* paramsDict, int replicaID);
};
