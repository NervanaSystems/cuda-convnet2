 ...
 *
 * * Added MinWeightConstraintOperator
 * ---------------------------------------------------------------------------
 */
...
class MinWeightConstraintOperator {
private:
    float _norm, _norm2;
public:
    MinWeightConstraintOperator(float norm) : _norm(norm), _norm2(norm*norm) {
    }
    __device__ inline float operator()(const float a) const {
        return a >= _norm2 ? 1.0f : __fdividef(_norm, sqrtf(a));
    }
};
